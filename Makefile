...
buildx-push: # Build and push single image image. Usage: make buildx-push POSTAL_VERSION="postalversion"
	@docker buildx build --no-cache --platform linux/amd64 --load --build-arg BUILD_DATE=`date -u +"%Y-%m-%dT%H:%M:%SZ"` --build-arg VCS_REF=`git rev-parse --short HEAD` --build-arg POSTAL_VERSION="$(POSTAL_VERSION)" --build-arg POSTAL_COMMIT="${POSTAL_COMMIT}" -t siebsie23/docker-postal:$(POSTAL_VERSION) .
